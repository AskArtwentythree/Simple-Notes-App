...
def test_sign_in_success(client, mock_db):
    ...
    response = client.post(
        "/sign_in", json={"username": "username", "password": "password"}
    )
...
def test_sign_in_invalid_credentials(client, mock_db):
    ...
    response = client.post(
        "/sign_in", json={"username": "username", "password": "wrongpass_password"}
    )
...
def test_sign_up_success(client, mock_db):
    ...
    response = client.post(
        "/sign_up",
        json={"email": "email", "username": "new_username", "password": "password"},
    )
...
def test_get_note_by_id_success(client, mock_db):
    ...
    response = client.get("/notes/1", headers={"Authorization": "Bearer valid-token"})
...
def test_delete_note_success(client, mock_db):
    ...
    response = client.delete(
        "/notes/1", headers={"Authorization": "Bearer valid-token"}
    )
